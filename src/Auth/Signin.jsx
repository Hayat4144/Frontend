...
import { Toast_Config_Option } from "../global/Toast_Config.jsx";
  ...
  const _GetCartItems = async () => {
    ...
    CartItem.then((item) => {
      if (Array.isArray(item) && item.length > 0) {
        item.map((i) => dispatch({ type: ADD_TO_CART, payload: i }));
      }
    }).catch((error) => console.error(error));
  };

  const _SendCartItems = async () => {
    if (CartData.length > 0) {
      const UnsendCartItem = CartData.filter(
        (i) => !i.hasOwnProperty("_id") && !i.hasOwnProperty("CartId")
      );
      const UnsendCartItemPromises = Promise.all(
        UnsendCartItem.map((items) => SendCartItemBackend({ ...items }))
      );
      UnsendCartItemPromises.then((data) => {
        if (data.length > 0) {
          data.map((item) =>
  ...
  return (
    <Fragment>
      <div className="Signin_container">
        ...
        >
          ...
          >
            ...
            <div className="password_field mx-4">
              ...
              />
              <h3
                className="text-sm cursor-pointer bg-red-500"
                onClick={(e) => {
                  setshowpassword(!showPassword);
                }}
              >
                <span className="relative bottom-[35px] float-right right-2">
        ...
        <div
          className="create_new_account sm:mx-auto  sm:w-[50%] mt-4 xl:mx-auto  
                lg:mx-auto lg:w-[40%] md:w-[50%] xl:w-[30%]"
        >
          <h3
            className="dont_have_account  mx-4 before:bg-gray-400 before:lg:w-[100px]
          ...
          <div className="new_accoutn_btn mx-4 my-10">
            <button
              className="w-full bg-indigo-800 py-2 px-6 rounded-md mb-2
